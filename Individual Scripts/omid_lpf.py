...
DESCRIPTION, VERSION, UA, TIMEOUT = 'Login page finder script', '0.2f', {'User-Agent': 'Mozilla/5.0 (X11; U; Linux x86_64) Gecko/20071127 Firefox/2.0.0.11'}, 30
...
class FindLoginPage():
	...
	def __init__ 	(
					...
					number_of_sub_links:'number of lines file links.txt contains'
					):
						self.domain = domain
	...
	def check (self):
		...
		for sub_link in self.sub_links:
			...
			try:
				...
				stdout.write("\r" + "No. of checked links: %s/%s" %(self.sub_link_counter, self.number_of_sub_links))
				stdout.flush()
...
def load_data(full_domain):
	...
	try:
		...
		print ("Error: file can't be opened, check if file 'links.txt' exist.")
		return
